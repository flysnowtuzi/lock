 ...
 * 
 */
app.controller('DeviceCtrl', ['$scope','DeviceService','mask','sysCode',function($scope,DeviceService,mask,sysCode) {
	console.log('请求进入');
	...
	$scope.deviceTypeList=sysCode.getTypeList('DeviceType');
	$scope.deviceTypeMap=sysCode.getTypeMap('DeviceType');	
	//sysCode.convertList2Map($scope.appIfTypeList);  不知为啥不好用，等待调试
	 ...
	 $scope.mutiSource=[{
			queryName : '事件类型',// 名称
			queryCode : "DeviceType",
			items :$scope.deviceTypeList
		},{
			queryName : '是否启用',// 名称
			queryCode : "isUseable",
			items :  [{id:'01',mc:'启用'},
	...
	//每个表单验证配置，如果不设置，默认和全局配置相同
	 ...
	 /*监控分页工具选择页码的变化，若变更则执行后台调用*/
	  ...
	  $scope.queryList = function(page) {
			mask.showLoading();
			//page.numPerPage=5;
			DeviceService.queryList(page,function(res) {
	...
	$scope.del=function(item){
		DeviceService.del(item,function(){
			$scope.queryList($scope.page);
		})
	}
	  //手动查询
	...
	$scope.isUseable=function(item){
		...
		//item.isUseable="02";
		DeviceService.update(item,function(){
			$scope.queryList($scope.page);
		});
	};
	//处理表单
	...
	$scope.saveEntity = function() {
		...
		if($scope.modelEdit){
			DeviceService.update($scope.app,function(){
				$scope.queryList($scope.page);
			});
			$scope.modelEdit=false;
			$scope.flag=false;
		}else{
			DeviceService.add($scope.app,function(){
				$scope.queryList($scope.page); 
			});
			$scope.modelAdd=false;
			$scope.flag=false;
...
app.service('DeviceService', ['http','mask','timeAlert','dialog', function(http,mask,timeAlert,dialog) {
	return {
		// 查询应用列表
		queryList : function(param,callback) {
			 // mask.showLoading();
			  http.callService('device/list',param,callback)
		...
		update:function(item,callback){
			  ...
			  http.callService('device/update',item,function(data){
		...
		add:function(item,callback){
			  ...
			  http.callService('device/add',item,function(data){
				if(data.statusCode=='200'){
		...
		del:function(item,callback){
			...
			dialog.confirm("确定要删除"+item.devName+"的设备吗?",function(){
	    		mask.showLoading();
				  http.callService('device/del',item,function(data){
					if(data.statusCode=='200'){
