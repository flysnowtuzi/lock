 ...
 * 
 */
app.controller('EventCtrl', ['$scope','EventService','mask','sysCode',function($scope,EventService,mask,sysCode) {
	console.log('请求进入');
	...
	//得到代码
	$scope.eventTypeMap=sysCode.getMap('EventType');
	$scope.eventTypeList=sysCode.getList('EventType');
	...
	//每个表单验证配置，如果不设置，默认和全局配置相同
	 ...
	 /*监控分页工具选择页码的变化，若变更则执行后台调用*/
	  ...
	  $scope.queryList = function(page) {
			mask.showLoading();
			//page.numPerPage=5;
			EventService.queryList(page,function(res) {
	...
	$scope.del=function(item){
		EventService.del(item,function(){
			$scope.queryList($scope.page);
		})
	}
	  //手动查询
	...
	$scope.isUseable=function(item){
		...
		//item.isUseable="02";
		EventService.update(item,function(){
			$scope.queryList($scope.page);
		});
	};
	//处理表单
	...
	$scope.saveEntity = function() {
		...
		if($scope.modelEdit){
			EventService.update($scope.app,function(){
				$scope.queryList($scope.page);
			});
			$scope.modelEdit=false;
			$scope.flag=false;
		}else{
			EventService.add($scope.app,function(){
				$scope.queryList($scope.page); 
			});
			$scope.modelAdd=false;
			$scope.flag=false;
...
app.service('EventService', ['http','mask','timeAlert','dialog', function(http,mask,timeAlert,dialog) {
	return {
		// 查询应用列表
		queryList : function(param,callback) {
			 // mask.showLoading();
			  http.callService('event/list',param,callback)
		...
		update:function(item,callback){
			  ...
			  http.callService('event/update',item,function(data){
		...
		add:function(item,callback){
			  ...
			  http.callService('event/add',item,function(data){
				if(data.statusCode=='200'){
		...
		del:function(item,callback){
			...
			dialog.confirm("确定要删除"+item.eventName+"的事件吗?",function(){
	    		mask.showLoading();
				  http.callService('event/del',item,function(data){
					if(data.statusCode=='200'){
