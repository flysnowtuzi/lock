 ...
 * 
 */
app.controller('LockCtrl', ['$scope','LockService','mask','sysCode',function($scope,LockService,mask,sysCode) {
	console.log('请求进入');
	...
	$scope.deviceList=sysCode.getDeviceList("11");
	$scope.deviceMap=sysCode.getDeviceMap('11');	
	...
	$scope.lockFlagList = lockFlagList;
	...
	$scope.lockStatusList = lockStatusList;
	...
	//每个表单验证配置，如果不设置，默认和全局配置相同
	 ...
	 /*监控分页工具选择页码的变化，若变更则执行后台调用*/
	  ...
	  $scope.queryList = function(page) {
			mask.showLoading();
			//page.numPerPage=5;
			LockService.queryList(page,function(res) {
	...
	$scope.isUseable=function(item){
		...
		//item.isUseable="02";
		LockService.update(item,function(){
			$scope.queryList($scope.page);
		});
	};
	//处理表单
	...
	$scope.saveEntity = function() {
		...
		if($scope.modelEdit){
			LockService.update($scope.app,function(){
				$scope.queryList($scope.page);
			});
			$scope.modelEdit=false;
			$scope.flag=false;
		}else{
			LockService.add($scope.app,function(){
				$scope.queryList($scope.page); 
			});
			$scope.modelAdd=false;
			$scope.flag=false;
...
app.service('LockService', ['http','mask','timeAlert','dialog', function(http,mask,timeAlert,dialog) {
	return {
		// 查询应用列表
		queryList : function(param,callback) {
			 // mask.showLoading();
			  http.callService('lock/list',param,callback)
