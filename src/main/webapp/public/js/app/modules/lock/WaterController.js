 ...
 * 
 */
app.controller('WaterCtrl', ['$scope','WaterService','mask','sysCode',function($scope,WaterService,mask,sysCode) {
	console.log('请求进入');
	...
	//每个表单验证配置，如果不设置，默认和全局配置相同
	 ...
	 /*监控分页工具选择页码的变化，若变更则执行后台调用*/
	  ...
	  $scope.queryList = function(page) {
			mask.showLoading();
			//page.numPerPage=5;
			WaterService.queryList(page,function(res) {
...
/*	//添加新的应用
	...
	$scope.del=function(item){
		WaterService.del(item,function(){
			$scope.queryList($scope.page);
		})
	}
	  //手动查询
	...
	$scope.isUseable=function(item){
		...
		//item.isUseable="02";
		WaterService.update(item,function(){
			$scope.queryList($scope.page);
		});
	};
	//处理表单
	...
	$scope.saveEntity = function() {
		...
		if($scope.modelEdit){
			WaterService.update($scope.app,function(){
				$scope.queryList($scope.page);
			});
			$scope.modelEdit=false;
			$scope.flag=false;
		}else{
			WaterService.add($scope.app,function(){
				$scope.queryList($scope.page); 
			});
			$scope.modelAdd=false;
			$scope.flag=false;
...
app.service('WaterService', ['http','mask','timeAlert','dialog', function(http,mask,timeAlert,dialog) {
	return {
		// 查询应用列表
		queryList : function(param,callback) {
			 // mask.showLoading();
			  http.callService('water/list',param,callback)
		...
		update:function(item,callback){
			  ...
			  http.callService('water/update',item,function(data){
		...
		add:function(item,callback){
			  ...
			  http.callService('water/add',item,function(data){
				if(data.statusCode=='200'){
		...
		del:function(item,callback){
			...
			dialog.confirm("确定要删除"+item.waterName+"的流水吗?",function(){
	    		mask.showLoading();
				  http.callService('water/del',item,function(data){
					if(data.statusCode=='200'){
